...
from utils.delta.delta import coef_inter, calc
...
if __name__ == '__main__':
    ...
    df_ml_train = df_ml.sample(frac=0.9, random_state=0)
    slope, intercept = coef_inter(df_ml_train)
    df_ml_train['dG_act_eyring'] = df_ml_train['dG_rxn'].apply(lambda x: calc(x, slope[0], intercept[0])[0])
    df_ml_train['ddG_act'] = df_ml_train['dG_act_corrected'] - df_ml_train['dG_act_eyring']
    df_ml_train.to_pickle('tmp/input_ffnn_delta_train.pkl')
    df_ml_test = df_ml.loc[~df_ml['rxn_id'].isin(df_ml_train['rxn_id'])]
    df_ml_test.to_csv('tmp/input_ffnn_delta_test.csv')
    # run_cv(data_path='tmp/input_ffnn_delta_train.pkl', target_column='ddG_act', save_dir='tmp/cv_delta_model')
    # mae, rmse, r2 = read_log('tmp/cv_delta_model/ffn_train.log')
    # logger.info(f'Delta Model')
    # logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation and 4 ensembles: {rmse} {mae} {r2}')
    run_train(save_dir='tmp/final_model_delta_4', data_path='tmp/input_ffnn_delta_train.pkl', transfer_learning=False, target_column='ddG_act', ensemble_size=4)
    run_reactivity(trained_dir='tmp/final_model_delta_4/', target_column='ddG_act', input_file='input_ffnn_delta_test.csv')
    df_pred = pd.read_csv('tmp/pred.csv')
    df_ml_test['dG_act_eyring'] = df_ml_test['dG_rxn'].apply(lambda x: calc(x, slope[0], intercept[0])[0])
    df_ml_test['ddG_act'] = df_pred['DG_TS_tunn'].tolist()
    df_ml_test['dG_act_eyring_delta'] = df_ml_test['dG_act_eyring'] + df_ml_test['ddG_act']
    mae, rmse, r2 = get_stats(df_ml_test['dG_act_corrected'], df_ml_test['dG_act_eyring'])
    logger.info(f'Eyring equation')
    logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation and 4 ensembles: {rmse} {mae} {r2}')
    mae, rmse, r2 = get_stats(df_ml_test['dG_act_corrected'], df_ml_test['dG_act_eyring_delta'])
    logger.info(f'Eyring equation Delta Model')
    logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation and 4 ensembles: {rmse} {mae} {r2}')
    ...
    # parameters_rf = {'n_estimators': 200, 'max_features': 0.7, 'min_samples_leaf': 1}
    ...
    # run_cv(target_column='G_act', save_dir='tmp/cv_omega_TF_4', k_fold=10, ensemble_size=4, transfer_learning=True, test_set='tmp/omega_data/test_set.csv', train_valid_set='tmp/omega_data/train_valid_set.csv', random_state=0, batch_size=24)
    # mae, rmse, r2 = read_log('tmp/cv_omega_TF_4/ffn_train.log')
    # logger.info("4 ensembles and transfer learning")
    # logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation: {rmse} {mae} {r2}')
    # run_cv(target_column='G_act', save_dir='tmp/cv_omega_no_TF_4', k_fold=10, ensemble_size=4, test_set='tmp/omega_data/test_set.csv', train_valid_set='tmp/omega_data/train_valid_set.csv', random_state=0, batch_size=24)
    # mae, rmse, r2 = read_log('tmp/cv_omega_no_TF_4/ffn_train.log')
    # logger.info("4 ensembles")
    # logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation: {rmse} {mae} {r2}')
    # run_cv(target_column='G_act', save_dir='tmp/cv_omega_no_TF_1', k_fold=10, ensemble_size=1, test_set='tmp/omega_data/test_set.csv', train_valid_set='tmp/omega_data/train_valid_set.csv', random_state=0, batch_size=24)
    # mae, rmse, r2 = read_log('tmp/cv_omega_no_TF_1/ffn_train.log')
    # logger.info("1 ensemble")
    # logger.info(f'10-fold CV RMSE, MAE and R^2 for NN with a learned-VB representation: {rmse} {mae} {r2}')
    # run_cv(target_column='G_act', save_dir='tmp/cv_omega_TF_1', k_fold=10, ensemble_size=1, transfer_learning=True, test_set='tmp/omega_data/test_set.csv', train_valid_set='tmp/omega_data/train_valid_set.csv', random_state=0, trained_dir='reactivity_model/results/final_model_1/', batch_size=24)
    ...
    # parameters_rf = {'n_estimators': 600, 'max_features': 0.8, 'min_samples_leaf': 1}
    # for sample in [25, 50, 100, 200, 300, 400]:
